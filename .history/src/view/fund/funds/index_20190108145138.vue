<template>
	 <div class="app">
		...
		<div class="below">
			...
			<div class="content">
				<p class="cont_title">{{yunInList.mining_name}}</p>
				<div class="cont_detail">
					<p>预估产出  0.00003350  BTC / T</p>
					<p>剩余 13,213.00 T</p>
				</div>
				<p class="price">{{(yunInList.price-0)|cutZreoFour}} <span>HKP / T</span><span class="buy_button fr" @click="jumpyunbuy">立即购买</span></p>
			</div>
		</div>
...
export default {
	...
	created() {
		...
		this.recommend();
		yunProList().then(res=>{
			this.yunInList=res.data.data[0]
		})
	},
}
</script>
