			...
			<div class="content">
				<div class="top">
					<ul class="topBar">
						...
						<li>
							<span>矿机算力</span>
								<input class="inText" placeholder="请输入购买的算力数值"  v-model="buyCalcT" @input="numinput"/>
							<span>T</span>
						</li>
						<li>
							<span>算力价格</span>
								<input class="inText" placeholder="请输入购买的算力单价"  v-model="buyCalcPrice" @input="numinput1"/>
				...
				<div class="bottom">
					<ul class="downBar">
						<li>
							<span>矿机功耗</span>
								<input class="inText" placeholder="请输入矿机的额定功耗"  v-model="powerDiss"  @input="numinput3"/>
							<span>W</span>
						</li>
						<li>
							<span>电价</span>
								<input class="inText" placeholder="请输入当前电价"  v-model="elePrice"  @input="numinput4"/>
							<span>{{coinType}}/T</span>
						</li>
						<li>
							<span>币产量</span>
...
<script>
   export default {
	...
	methods:{
		...
		numinput1(e){
			this.buyCalcPrice = e.target.value.replace(/[^\d.]/g,'')
		},
		numinput2(e){
			this.buyCalcDays = e.target.value.replace(/[^\d]/g,'')
		},
		numinput3(e){
			this.powerDiss = e.target.value.replace(/[^\d]/g,'')
		},
		numinput4(e){
			this.elePrice = e.target.value.replace(/[^\d]/g,'')
		},
		numinput5(e){
			this.coinOut = e.target.value.replace(/[^\d]/g,'')
		},
		numinput6(e){
			this.coinPirce = e.target.value.replace(/[^\d]/g,'')
		},
		calcHelp(){
			this.calcWay = true;
		},
